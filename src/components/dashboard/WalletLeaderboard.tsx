          ...
          {isLoading ? (
            ...
            <div className="space-y-1 px-2">
              {filteredWallets.map((wallet, index) => (
                ...
                >
                  {index < 3 ? (
                    <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold ${rankColors[index]}`}>
                      {index + 1}
                    </span>
                  ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      {wallet.isEdgeTrader && (
                        <Badge className="badge-edge h-4">EDGE</Badge>
                      )}
                    </div>
                    <div className="mt-0.5 flex items-center gap-3 text-[11px] text-[#64748b]">
                      ...
                      <span className={wallet.totalPnl >= 0 ? 'text-[#00ff41] font-semibold' : 'text-[#ef4444] font-semibold'}>
                        {wallet.totalPnl >= 0 ? '+' : ''}{formatPnl(wallet.totalPnl)}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="h-1 w-12 overflow-hidden rounded-full bg-[#1e293b]">
                      <div
                        ...
                        style={{ width: `${Math.min(wallet.edgeScore * 100, 100)}%` }}
                      />
                    </div>
                  </div>
                </div>
