...
async function run() {
  ...
  if (optiMatch.match) {
    console.log(
      `  Match Reason: Triggered by ${optiMatch.match.author}'s tweet ${Math.round((Date.now() - optiMatch.match.timestamp) / 1000)} seconds ago.`,
    );
  }

  // 5. Test Narrative Distance on a generic token
  console.log("\n[5] Rust Sniper detects: Ticker: SHIBX, Name: ShibaX");
  const genericMatch = await engine.calculateNarrativeDistance(
    "ShibaX",
    "SHIBX",
  );
  console.log(
    `  Distance: ${genericMatch.distance.toFixed(1)} (100 = No relation)`,
  );
  if (!genericMatch.match) {
    console.log(
      `  Match Reason: No active narrative correlation found in the sliding window.`,
    );
  }
