...
export async function fetchSolanaTokenInfo(address: string, rpcUrl: string = "https://api.mainnet-beta.solana.com") {
  try {
    ...
    try {
      ...
      console.log("Could not fetch Solana creator for", address, (e as Error).message);
    }
    ...
    console.error(`Error fetching Solana token info for ${address}:`, (error as Error).message);
...
export async function fetchEvmTokenInfo(address: string, rpcUrl: string) {
  try {
    ...
    console.error(`Error fetching EVM token info for ${address}:`, (error as Error).message);
