...
Define reusable pipelines in `.opencode-flow/config.json`:
...
# Results stored in .opencode-flow/memory/stream-chain/
```
...
cat .opencode-flow/config.json | grep -A 10 "streamChain"
```
