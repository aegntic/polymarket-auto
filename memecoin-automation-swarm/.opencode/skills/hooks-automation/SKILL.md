...
- `.opencode/settings.json` with hook configurations
...
This creates:
- `.opencode/settings.json` with pre-configured hooks
- Hook command documentation in `.opencode/commands/hooks/`
...
Edit `.opencode/settings.json` to configure hooks:
...
```javascript
// .opencode/hooks/custom-quality-check.js
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "script",
            "script": ".opencode/hooks/custom-quality-check.js"
          }
        ]
      }
    ]
...
cat .opencode-flow/logs/hooks-$(date +%Y-%m-%d).log
...
#### Hooks Not Executing
- Verify `.opencode/settings.json` syntax
