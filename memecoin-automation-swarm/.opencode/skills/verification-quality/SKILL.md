...
npx claude-flow@alpha truth --export .opencode-flow/metrics/truth-$(date +%Y%m%d).json
```
...
Set verification preferences in `.opencode-flow/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".opencode-flow/metrics/truth-daily.json"
    }
  }
}
```
